...
import { getJobCategories } from 'apiSdk/job-categories';
...
import { JobListingListPage as ShowCaseList } from 'pages/job-listings';

function DashboardPage() {
  const { session } = useSession();

  const jobApplicationStat = useSWR<PaginatedInterface<JobApplicationInterface>>(
    () => `/job-applications`,
    () => getJobApplications({ limit: 1, offset: 0 }),
  );

  const jobCategoryStat = useSWR<PaginatedInterface<JobCategoryInterface>>(
    () => `/job-categories`,
    () => getJobCategories({ limit: 1, offset: 0 }),
  );

  const jobListingStat = useSWR<PaginatedInterface<JobListingInterface>>(
    () => `/job-listings`,
    () => getJobListings({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Job Applications',
      result: jobApplicationStat?.data?.totalCount,
      isLoading: jobApplicationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Job Categories',
      result: jobCategoryStat?.data?.totalCount,
      isLoading: jobCategoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Job Listings',
      result: jobListingStat?.data?.totalCount,
      isLoading: jobListingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`job-listings`} passHref legacyBehavior>
                  <Button
